...
const GameScreen = ({ settings, onGameEnd }) => {
  ...
  // Sound effects
  const playSound = useCallback((soundId) => { // Wrapped in useCallback for stability if passed as prop or used in other effects
  ...
  const initializeQuiz = useCallback(() => {
    ...
    for (let i = 0; i < settings.numProblems; i++) {
      newProblems.push(generateProblem(settings.numDigits, settings.selectedOp));
    }
  ...
  const handleSubmitQuiz = useCallback(() => { // Wrapped handleSubmitQuiz in useCallback
    ...
    const results = problems.map(p => ({
      problem: p,
      userAnswer: userAnswers[p.id] === '' ? null : parseInt(userAnswers[p.id], 10),
  ...
  useEffect(() => {
    if (settings.timerDuration === 0 || timeLeft <= 0 || problems.length === 0 || isSubmitted) {
  ...
  }, [timeLeft, settings.timerDuration, problems, isSubmitted, playSound, handleSubmitQuiz]); // Added handleSubmitQuiz here
  ...
  const handleRestartQuiz = useCallback(() => { // Also wrap this in useCallback for consistency
  ...
  if (problems.length === 0) {
    return <div className="text-center text-xl text-gray-600 p-10">Setting up your quiz... ⚙️</div>;
  }
  return (
    <div className="w-full max-w-2xl mx-auto p-6 bg-white bg-opacity-95 rounded-xl shadow-2xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        {settings.timerDuration > 0 && (
          <div className={`text-xl font-bold p-3 rounded-lg shadow ${timeLeft <= 10 && timeLeft > 0 ? 'text-red-500 animate-pulse bg-red-100' : 'text-indigo-600 bg-indigo-100'}`}>
      ...
      <div className="space-y-5 max-h-[60vh] overflow-y-auto pr-2">
        {problems.map((p, index) => (
          <div key={p.id} className="flex flex-col sm:flex-row items-center gap-3 p-4 bg-blue-50 border border-blue-200 rounded-lg shadow-sm">
